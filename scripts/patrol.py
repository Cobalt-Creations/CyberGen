...
class Patrol():
    ...
    def add_new_cats(self, outcome, success):
        ...
        if cat_type == 'kittypet' or "kittypet" in attribute_list:
            ...
            if not other_clan:
                ...
                if not success:
                    outsider = create_outside_cat(Cat, loner, backstory=choice(backstory))
                    self.results_text.append(f"The team has met {outsider}.")
                    return
            else:
                if not success:
                    outsider = create_outside_cat(Cat, "former Clancat", backstory=choice(backstory))
        ...
        if "dead" in attribute_list:
            ...
            alive = False
            thought = "Is glad that their sparklings are safe"
        ...
        if f"litter{outcome}" in attribute_list:
            ...
            if "litternewborn" in attribute_list:
                ...
                kit_age = 0
                kit_thought = "Beeps quietly for energon"
            else:
            ...
            if game.clan.game_mode != 'classic' and kit_age <= 2 and not created_cats[0].dead:
                if not game.settings["no gendered breeding"]:
                    if created_cats[0].gender == 'female':
                        created_cats[0].get_injured("recovering from forging sparklings")
                else:
                    created_cats[0].get_injured("recovering from forging sparklings")
        ...
        for new_cat in created_cats:
            ...
            self.results_text.append(f"{new_cat.name} has joined the team.")
            ...
            if cat.status == 'leader':
                if 'all_lives' in self.patrol_event.tags:
                    ...
                    if game.clan.leader_lives > 2:
                        ...
                        self.results_text.append(f"{cat.name} was revived by the Matrix multiple times.")
                    else:
                        game.clan.leader_lives -= 1
                        self.results_text.append(f"{cat.name} had to be revived by the Matrix.")
                else:
                    game.clan.leader_lives -= 1
    ...
    def handle_prey(self, outcome_nr):
            ...
            if len(relevant_patrol_tags) == 0:
                ...
                if len(patrol.patrol_cats) == 1:
                    self.results_text.append(
                        f"{self.patrol_leader_name} still manages to bring home some amount of fuel.")
                else:
                    self.results_text.append(f"The patrol still manages to bring home some amount of fuel.")
            return
        ...
        if game.clan.game_mode != "classic":
            ...
            if total_amount > 0:
                if len(patrol.patrol_cats) == 1:
                    self.results_text.append(f"{patrol.patrol_leader_name} brings back a {prey_size} amount of fuel.")
                else:
                    self.results_text.append(f"Each cat brings back a {prey_size} amount of energon.")
    ...
    def handle_reputation(self, difference):
        ...
        if difference > 0:
            ...
            insert = "worsened"
        self.results_text.append(f"Your team's reputation towards Outsiders has {insert}.")
