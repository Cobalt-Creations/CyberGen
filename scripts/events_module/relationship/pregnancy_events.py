...
class Pregnancy_Events():
    ...
    def handle_zero_moon_pregnant(self, cat, other_cat=None, relation=None, clan=game.clan):
        ...
        if not other_cat and cat.gender == 'male':
            ...
            if amount == 1:
                insert = 'a single sparkling'
            if amount > 1:
                insert = f'a group of {amount} sparklings'
    ...
    def handle_two_moon_pregnant(self, cat, clan=game.clan):
        ...
        if kits_amount == 1:
            insert = 'single sparkling'
        else:
            insert = f'group of {kits_amount} sparklings'
        ...
        if not int(random.random() * death_chance):  # chance for a cat to die during childbirth
            ...
            if cat.status == 'leader':
                clan.leader_lives -= 1
                cat.die()
                cat.died_by.append(f" died shortly after forging sparklings.")
            else:
                cat.die()
                cat.died_by.append(f"{cat.name} died while forging sparklings.")
