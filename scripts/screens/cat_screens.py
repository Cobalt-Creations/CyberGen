...
class ProfileScreen(Screens):
        ...
        # Dangerous Tab
            ...
            if event.ui_element == self.kill_cat_button:
                ...
                self.the_cat.died_by.append(
                    f'It was the will of something even mightier than the Allspark that this cat died.')
                ...
                if self.the_cat.dead:
                    if self.the_cat.df is True:
                        ...
                        self.the_cat.thought = "Is relieved to once again be in the Allspark"
                    else:
                        self.the_cat.outside, self.the_cat.exiled = False, False
                        self.the_cat.df = True
                        game.clan.add_to_darkforest(self.the_cat)
                        self.the_cat.thought = "Is distraught after being sent to the Pit"
    ...
    def clear_profile(self):
        ...
        if self.user_notes:
            self.user_notes = 'Click the check mark to enter notes about your bot!'
    ...
    def build_profile(self):
        ...
        if is_sc_instructor:
            self.the_cat.thought = "Hello. I am here to guide the dead bots of " + game.clan.name + "Clan into the Allspark."
        if is_df_instructor:
            self.the_cat.thought = "Hello. I am here to drag the dead bots of " + game.clan.name + "Clan into The Pit."
    ...
    def update_toggle_buttons(self):
        ...
        if self.the_cat.dead and game.settings["fading"]:
            ...
            self.checkboxes["prevent_fading"] = UIImageButton(scale(pygame.Rect((200, 770), (68, 68))), "",
                                                              starting_height=2,
                                                              tool_tip_text="Prevents a bot from fading away."
                                                                            " If unchecked, and the bot has been dead "
                                                                            "for longer than 202 solar cycles, they will fade "
    ...
    def generate_column1(self, the_cat):
        ...
        if the_cat.dead:
            ...
            if the_cat.moons == 1:
                output += ' solar cycle (in life)\n'
            elif the_cat.moons != 1:
                output += ' solar cycles (in life)\n'

            output += str(the_cat.dead_for)
            if the_cat.dead_for == 1:
                output += ' solar cycle (in death)'
            elif the_cat.dead_for != 1:
                output += ' solar cycles (in death)'
        else:
            output += str(the_cat.moons)
            if the_cat.moons == 1:
                output += ' solar cycle'
    ...
    def generate_column2(self, the_cat):
        ...
        if the_cat.is_injured():
            if "recovering from birth" in the_cat.injuries:
                output += 'recovering from forging!'
            elif "pregnant" in the_cat.injuries:
                output += 'carrying sparklings!'
            else:
                output += "injured!"
        elif the_cat.is_ill():
    ...
    def save_user_notes(self):
        ...
        try:
            ...
            print(f"WARNING: Saving notes of bot #{self.the_cat.ID} didn't work.")
    ...
    def load_user_notes(self):
        ...
        try:
            ...
            print(f"ERROR: there was an error reading the Notes file of bot #{self.the_cat.ID}.\n", e)
    ...
    def get_backstory_text(self):
        ...
        if bs_blurb is not None:
            adjust_text = str(bs_blurb).replace('This bot', str(self.the_cat.name))
            text = adjust_text
        else:
    ...
    def get_influence_text(self):
        ...
        if mentor is None:
            influence_history = "This bot either did not have a mentor, or their mentor is unknown."
            if self.the_cat.status == 'kitten':
                influence_history = 'This bot has not begun training.'
            if self.the_cat.status in ['apprentice', 'medicine cat apprentice']:
                influence_history = 'This bot has not finished training.'
        elif influenced_skill is not None and influenced_trait is None:
            influence_history = f"The influence of their mentor, {mentor}, caused this bot to {influenced_skill}."
            ...
            if influenced_trait in ['Outgoing', 'Benevolent', 'Abrasive', 'Reserved']:
                influence_history = f"The influence of their mentor, {mentor}, caused this bot to become more {influenced_trait.lower()}."
            else:
                influence_history = f"This bot's mentor was {mentor}."
        elif influenced_trait is not None and influenced_skill is not None:
            influence_history = f"The influence of their mentor, {mentor}, caused this bot to become more {influenced_trait.lower()} as well as {influenced_skill}."
        else:
            influence_history = f"This bot's mentor was {mentor}."
    ...
    def get_condition_details(self, name):
        ...
        if name in self.the_cat.permanent_condition:
            ...
            if self.the_cat.permanent_condition[name]["born_with"] is True:
                ...
                if moons_with != 1:
                    text_list.append(f"has had this condition for {moons_with} solar cycles")
                else:
                    text_list.append(f"has had this condition for 1 solar cycle")
        ...
        if name in self.the_cat.injuries:
            ...
            if 'moons_with' in keys:  # need to check if it exists for older saves
                ...
                if name == 'recovering from birth':
                    ...
                    insert = 'has been carrying for'
                if moons_with != 1:
                    text_list.append(f"{insert} {moons_with} solar cycles")
                else:
                    text_list.append(f"{insert} 1 solar cycle")
            # infected or festering
            ...
            if not self.the_cat.dead:
                ...
                if self.the_cat.dead and game.clan.instructor.ID == self.the_cat.ID:
                    self.exile_cat_button = UIImageButton(scale(pygame.Rect((1156, 900), (344, 92))),
                                                          "",
                                                          object_id=object_id,
                                                          tool_tip_text='Changing where this bot resides will change '
                                                                        'where your team goes after death. ',
                                                          starting_height=2, manager=MANAGER)
                else:
                    self.exile_cat_button = UIImageButton(scale(pygame.Rect((1156, 900), (344, 92))),
                                                          "",
    ...
    def create_leadership_ceremony(self, cat):
        ...
        if len(cat.life_givers) == 0:
            ...
            prev_lead_virtues = ["endurance in the face of hardship", "knowing when to fight and when to choose peace",
                                 "leadership through the darkest times", "loyalty to their team",
            ...
            if not med_cat:
                med_cat = choice(names.names_dict["normal_prefixes"]) + \
                          choice(names.names_dict["normal_suffixes"])
            if not prev_lead:
                prev_lead = choice(names.names_dict["normal_prefixes"]) + "imus Prime"
...
class RoleScreen(Screens):
    ...
    def screen_switches(self):
        ...
        self.retire = UIImageButton(scale(pygame.Rect((451, 792), (334, 72))), "",
                                    ...
                                    tool_tip_text="If a cat is retired, you will be "
                                                  "unable to switch them to soldier status. ",
                                    manager=MANAGER)
    ...
    def get_role_blurb(self):
        if self.the_cat.status == "warrior":
            output = f"{self.the_cat.name} is a <b>soldier</b>. Soldiers are adult bots who feed and protect their " \
                     f"team. They are trained to hunt and fight in addition to the ways of surviving in this world. " \
                     f"Soldiers are essential to the survival of a team, and usually make up the bulk of it's members. "
        elif self.the_cat.status == "leader":
            output = f"{self.the_cat.name} is the <b>leader</b> of {game.clan.name}Clan. The guardianship of all " \
                     f"these bots has been entrusted to them by the Matrix. The leader is the highest " \
                     f"authority in the team. The leader holds meetings, determines mentors for " \
                     f"new bots, and holds ceremonies for new soldiers. To help them protect the team, " \
                     f"the Matrix will heal them from death nine times. They typically are also called \"Primes\"."
        elif self.the_cat.status == "deputy":
            output = f"{self.the_cat.name} is {game.clan.name}Clan's <b>deputy</b>. " \
                     ...
                     f"alongside normal soldier duties. Typically, a deputy is personally appointed by the current " \
                     f"leader. All deputies must train at least one apprentice, as a test for their leadership " \
                     f"skills before appointment.  " \
                     f"The deputy succeeds the leader if they die or retire. "
        elif self.the_cat.status == "medicine cat":
            output = f"{self.the_cat.name} is a <b>medic</b>. Medics are the healers of the team. " \
                     f"They treat " \
                     f"injuries and illnesses with various tools and materials. Unlike soldiers, medics are not expected " \
                     f"to forage and fight for the team. In addition to their healing duties, medics often have " \
                     f"a special connection to the Allspark. Ghosts often visit them to give them advice and help " \
                     f"beyond the grave. But not all ghosts can be trusted... "
        elif self.the_cat.status == "mediator":
            output = f"{self.the_cat.name} is a <b>mediator</b>. Mediators are not typically required " \
                     f"to forage or fight for " \
                     f"the team. Rather, mediators are charged with handling disagreements between " \
                     f"teammates and disputes with other groups. Some mediators train as cadets to serve their team, " \
                     f"while others may choose to become mediators later in life. "
        elif self.the_cat.status == "elder":
            output = f"{self.the_cat.name} is an <b>elder</b>. They have spent many solar cycles serving their team, " \
                     f"and have earned " \
                     f"many solar cycles of rest. Elders are essential to passing down the wisdom of the past. " \
                     f"Sometimes, bots may retire due to disability or injury. Whatever the " \
                     f"circumstance of their retirement, elders are held in high esteem in the team, and always eat " \
                     f"before soldiers and Medics. "
        elif self.the_cat.status == "apprentice":
            output = f"{self.the_cat.name} is a <b>cadet</b>, in training to become a soldier. " \
                     f"Sparklings can be made cadets at six solar cycles of age, where they will learn how " \
                     f"to forage and fight for their team. Typically, the training of an cadet is entrusted " \
                     f"to an single soldier - their mentor. To build character, cadets are often assigned " \
                     f"the unpleasant and grunt tasks of team life. Cadets also are the youngest bots " \
                     f"to have gotten their alt mode though their frame is not done growing yet. "
        elif self.the_cat.status == "medicine cat apprentice":
            output = f"{self.the_cat.name} is a <b>medic in training</b>, hoping to get their badge. " \
                     f"Sparklings can be made medics at six solar cycles of age, where they will learn how to " \
                     f"heal their teammates and commune with spirits. Medics are typically chosen " \
                     f"for their interest in healing and/or their connection to the Allspark. They have also picked an " \
                     f"alt mode for themselves now and can transform finally."
        elif self.the_cat.status == "mediator apprentice":
            output = f"{self.the_cat.name} is a <b>mediator apprentice</b>, training to become a full mediator. " \
                     f"Mediators are in charge of handling disagreements both within the team and between other groups. " \
                     f"Mediator apprentices are often chosen for their quick thinking and steady personality. " \
                     f"Maybe if they grow up to be the best mediator out there " \
                     f"they could be the key to ending the Autobot Decepticon War. "
        elif self.the_cat.status == "kitten":
            output = f"{self.the_cat.name} is a <b>sparkling</b>. All bots below the age of six solar cycles are " \
                     f"considered sparklings. Sparklings " \
                     f"are prohibited from leaving camp in order to protect them from the dangers of the city. " \
                     f"Although they don't have any official duties in the team, they are expected to learn the " \
                     f"legends and traditions of their team. They are protected by every bot in on the team and always " \
                     f"eat first. Sparklings have yet to take upon an alt mode yet."
        elif self.the_cat.status == "newborn":
            output = f"{self.the_cat.name} is a <b>newly forged sparkling</b>. All bots below the age of six solar cycles are " \
                     f"considered sparklings. Sparklings " \
                     f"are prohibited from leaving camp in order to protect them from the dangers of the war. " \
                     f"Although they don't have any official duties in the team, they are expected to learn the " \
                     f"legends and traditions of their team. They are protected by every bot on the team and always " \
                     f"eat first. They hardly have any armor nor an alt mode."
        else:
