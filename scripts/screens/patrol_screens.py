...
class PatrolScreen(Screens):
    ...
    def open_choose_cats_screen(self):
        ...
        self.elements["info"] = pygame_gui.elements.UITextBox(
            'Choose up to six bots to take on patrol.\n'
            'Smaller patrols help bots gain more experience, but larger patrols are safer.',
    ...
    def adjust_patrol_text(self, text, size=1):
        ...
        text = text.replace('o_c_n', str(other_clan_name) + 'cons')
            ...
            if 'c_n' in text:
                for y in vowels:
                    if str(clan_name).startswith(y):
                        ...
                        text = " ".join(modify)
                        break
        text = text.replace('c_n', str(game.clan.name) + 'Bots')
