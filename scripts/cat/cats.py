...
class Cat():
    ...
    def die(self, body: bool = True):
        ...
        if self.status == 'leader':
            if game.clan.leader_lives > 0:
                self.thought = 'Was startled to find themselves in the Allspark for a moment... did the Matrix save them?'
                return ""
            elif game.clan.leader_lives <= 0:
                self.dead = True
                game.clan.leader_lives = 0
                self.thought = 'Is surprised to find themselves one with the Allspark'
                if game.clan.instructor.df is False:
                    text = 'They\'ve died and joined the Allspark.'
                else:
                    text = 'They\'ve has lost their last life and have travelled to the Dark Forest.'
        else:
            self.dead = True
            self.thought = 'Is surprised to find themselves in the Allspark'
    ...
    def add_to_clan(self):
        ...
        if self.status in ['leader', 'deputy']:
            ...
            game.cur_events_list.append(Single_Event('A long overdue ceremony is held for ' + str(
                self.name) + '. They smile as they finally become a soldier and are now sporting a shiny badge ' + str(
            ...
            involved_cats = [self.ID]
            game.cur_events_list.append(Single_Event('A long overdue ceremony is held for ' + str(
                self.name) + '. They smile as they finally become a soldier and now have a brand new badge ' + str(
                self.name) + '.', "ceremony", involved_cats))
        elif self.status == 'kitten' and self.moons >= 6:
            self.status_change('apprentice')
            involved_cats = [self.ID]
            game.cur_events_list.append(Single_Event('A long overdue cadet ceremony is held for ' + str(
                self.name.prefix) + '. They smile as they finally become a cadet and are now sporting a new alt mode ' + str(
                self.name) + '.', "ceremony", involved_cats))
        ...
        if len(names) > 0:
            ...
            game.cur_events_list.append(Single_Event(f"Together with {self.name}, {str(event_text)} joins the team.", ids))
                    ...
                    if chance >= 9:
                        for x in possible_groups:
                            if mentor.skill in self.skill_groups[x]:
                                ...
                                self.skill = choice(possible_skill)
                                skill_influence = self.skill
                                History.add_skill_mentor_influence(self, mentor, skill_influence, trait=None)
                                return
    ...
    def moon_skip_illness(self, illness):
        ...
        if mortality and not int(random.random() * mortality):
            if self.status == "leader":
                ...
                game.clan.leader_lives -= 1
                if game.clan.leader_lives > 0:
                    text = f"{self.name} almost died to {illness}."
                    ...
                    text = f"{self.name} was killed by {illness}."
                    # game.health_events_list.append(text)
                    # game.birth_death_events_list.append(text)
                    game.cur_events_list.append(Single_Event(text, ["birth_death", "health"], game.clan.leader.ID))
            self.die()
